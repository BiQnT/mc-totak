...
- Java 8 사용 중 확인 (Launcher 자동 처리)
- Forge 1.16.5-21.1.x 정확한 버전 사용 (1.21.0이나 1.21.4 X)
- Pixelmon jar이 정확히 1.16.5 버전인지 (`Pixelmon-1.16.5-9.x.x-universal.jar`)
- mods 폴더에 다른 모드와 충돌 가능성 — 일단 Pixelmon 외 다른 모드 다 빼고 시도

## ❌ "Outdated client" / "Outdated server"
- 클라이언트 1.16.5 정확히 (1.21.0, 1.21.4 등 X)
...
- 서버에 무엇을 사용 중인지: `Pixelmon Reforged 9.1.13`
- 클라이언트도 동일 버전 9.3.14 또는 9.3.x로 맞추기

## ❌ Forge installer 실행 안 됨
- 다운된 jar이 실제 jar인지 확인 (브라우저가 zip으로 변경 안 했나)
- 우클릭 "Open With" → Java 8 선택
- 또는 명령줄: `java -jar neoforge-36.2.39-installer.jar`
...
## ❌ Pixelmon 메뉴/UI 안 보임
- F3 디버그 화면에서 "FML" 표시 확인 (Forge가 로드되었는지)
- Mods 메뉴에서 Pixelmon 활성 상태 확인

## ❌ 그래도 안 되면
- 본인 OS, MC 버전, Forge 버전, Pixelmon jar 파일명 + 정확한 오류 메시지를
